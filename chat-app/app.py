...
app.secret_key = os.environ.get("SECRET_KEY", "secret")  # optional secure secret
socketio = SocketIO(app, cors_allowed_origins="*", async_mode="eventlet")  # eventlet for deployment
...
def get_messages(user2):
    ...
    filtered = [m for m in messages if (m["from"] == user1 and m["to"] == user2) or (m["from"] == user2 and m["to"] == user1)]
...
# Run server on Replit-friendly host/port
